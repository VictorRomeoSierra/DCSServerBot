...
from .listener import SlotManagementEventListener


class SlotManagement(Plugin):
    """
    ...
    @command(description='This is a slotmanagement command.')
    @app_commands.guild_only()
    @utils.app_has_role('DCS')
    async def slotmanagement(self, interaction: discord.Interaction,
        ...
        data = await server.send_to_dcs_sync({
            "command": "slotmanagement",    # command name
...
async def setup(bot: DCSServerBot):
    await bot.add_cog(SlotManagement(bot, SlotManagementEventListener))
